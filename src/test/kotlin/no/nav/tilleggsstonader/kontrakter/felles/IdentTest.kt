...
class IdentTest {
    ...
    internal fun `skal tillate helsyntetiske nummer fra dolly`() {
        ...
        listeAvBrukere.forEach {
            assertThat(it.fnr).isEqualTo(Ident(it.fnr).verdi)
                .withFailMessage { "Fødselsnummer ${it.fnr} er gyldig" }
            assertThat(it.dnr).isEqualTo(Ident(it.dnr).verdi)
                .withFailMessage { "Dnr ${it.dnr} er gyldig" }
            assertThat(it.fødselsdato).isEqualTo(Ident(it.fnr).fødselsdato)
                .withFailMessage { "Finner dato for ${it.fnr}" }
            assertThat(it.fødselsdato).isEqualTo(Ident(it.dnr).fødselsdato)
                .withFailMessage { "Finner dato for ${it.dnr}" }
            assertThat(false).isEqualTo(Ident(it.fnr).erDNummer)
                .withFailMessage { "${it.fnr} er ikke D-nummer" }
            assertThat(true).isEqualTo(Ident(it.dnr).erDNummer)
